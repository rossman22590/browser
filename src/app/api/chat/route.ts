...
export async function POST(req: Request) {
  ...
  if (lastMessage?.role === "user" && process.env.NODE_ENV !== "development") {
    ...
    // Updated ratelimit call using a type cast to bypass TS check
    const { success, limit, reset, remaining } = await ratelimit.limit(ip, {
      max: 10000,    // Increased daily request limit
  ...
  // Initialize step counter for tracking execution steps
  let stepCount = 0;
    ...
    execute: async (dataStream) => {
      const result = streamText({
        ...
        tools: {
          ...
          viewAllClickableElements: {
            description:
              "Highlight all clickable elements on the page with indexes and bounding boxes. Use this to see what you can click on the page, then select the index of the element you want to click.",
            parameters: z.object({}),
            execute: async () => {
              const { page } = await getOrCreateBrowser(sessionId);
          ...
          browserAction: {
            ...
            execute: async ({ action, text, amount, clickIndex, wait }) => {
              try {
                ...
                if (action === "wait") {
                  if (!wait)
                    throw new Error("Wait parameter required for wait action");
                  await sleep(wait);
                  const { screenshot } = await takeScreenshot(page);
                  return {
                    data: screenshot.data,
                ...
                if (action === "click") {
                  if (clickIndex === undefined || clickIndex === null)
                    throw new Error("Click index parameter required for click action");
