...
export function ChatPanel({
  ...
  // Enhanced message handling with status
  ...
  const handleSubmit = React.useCallback(
    async (value: string) => {
      if (!sessionId || isProcessing || !value.trim()) return;
    ...
    [sessionId, isProcessing, append, setInput, shouldAutoScroll]
  );

  // Initial message handling
  React.useEffect(() => {
    if (initialMessage && sessionId && !messages.length) {
      handleSubmit(initialMessage);
    }
  }, [sessionId, initialMessage, messages.length, handleSubmit]);
        ...
        <div className="flex h-full flex-col p-4">
          ...
          <div className="mt-4">
            <ChatInput
              ...
              disabled={
                isLoading || isInitializing || !sessionId || isProcessing
              }
            />
            {isProcessing && (
